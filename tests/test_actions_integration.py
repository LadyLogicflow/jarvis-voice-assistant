"""Integration tests for `server.execute_action`.
...
async def _exec(action_type: str, payload: str = "") -> str:
    return await server.execute_action({"type": action_type, "payload": payload})
...
async def test_search_action_returns_summary():
    ...
    with patch.object(server.browser_tools, "search_and_read", new=AsyncMock(return_value=fake)):
...
async def test_browse_action_returns_summary():
    ...
    with patch.object(server.browser_tools, "visit", new=AsyncMock(return_value=fake)):
...
async def test_open_action_success_returns_geoeffnet():
    with patch.object(server.browser_tools, "open_url",
...
async def test_news_action_returns_text():
    with patch.object(server.browser_tools, "fetch_news",
...
async def test_mail_action_no_mails():
    with patch.object(server.mail_tools, "get_unread_mails", return_value="KEINE_MAILS"):
...
async def test_mail_action_with_unread():
    ...
    with patch.object(server.mail_tools, "get_unread_mails", return_value=canned):
        result = await _exec("MAIL")
...
async def test_tasks_action_no_token():
    server.TODOIST_TOKEN = ""  # explicit empty
    result = await _exec("TASKS")
    assert "nicht konfiguriert" in result


async def test_tasks_action_with_token(monkeypatch):
    monkeypatch.setattr(server, "TODOIST_TOKEN", "fake-token")
    with patch.object(server.todoist_tools, "get_tasks",
...
async def test_addtask_action_with_due(monkeypatch):
    ...
    with patch.object(server.todoist_tools, "add_task", new=fake_add):
...
async def test_addtask_action_without_due(monkeypatch):
    monkeypatch.setattr(server, "TODOIST_TOKEN", "fake-token")
    with patch.object(server.todoist_tools, "add_task",
...
async def test_donetask_action(monkeypatch):
    monkeypatch.setattr(server, "TODOIST_TOKEN", "fake-token")
    with patch.object(server.todoist_tools, "complete_task",
...
async def test_calendar_action_returns_events():
    with patch.object(server.google_calendar_tools, "get_events",
...
async def test_addcal_action_with_when():
    ...
    with patch.object(server.google_calendar_tools, "add_event", new=fake_add):
...
async def test_note_action_with_body():
    with patch.object(server.notes_tools, "add_note",
...
async def test_steuernews_uses_cached_brief():
    ...
    server.STEUER_BRIEF = "Cached brief from earlier today"
    server.STEUER_BRIEF_DATE = dt.date.today().isoformat()
...
async def test_steuernews_refreshes_when_no_cache():
    import datetime as dt
    server.STEUER_BRIEF = ""
    server.STEUER_BRIEF_DATE = ""

    async def fake_refresh():
        server.STEUER_BRIEF = "Fresh brief"
        server.STEUER_BRIEF_DATE = dt.date.today().isoformat()

    with patch.object(server, "refresh_steuer_brief", new=fake_refresh):
        result = await _exec("STEUERNEWS")
